...
exports.query = function (queries, extendedHeaders) {
  var deferred = q.defer(),
      promises = [];

  // if no headers were provided
  ...
  queries.forEach(function (query, i) {
    ...
    if (exports[query.type]) {
      if (query.map) {
        handleQueryMap(query, extendedHeaders, deferred, i);
      } else {
        handleQuery(query, extendedHeaders, deferred, i);
      }
    } else {
...
function handleQuery (query, extendedHeaders, deferred, i) {
  exports[query.type].fetch(query.url, query, extendedHeaders)
...
function handleQueryMap (query, extendedHeaders, deferred, i) {
  map(query, extendedHeaders, function (error, result) {
    if (!error) {
      result.orderNo = i;
      if (query.cache) {
        result.created = resultsCache.get(query).created;
...
/**
 * an array of function(query, requestOptions, next), invoked serially before each fetch
 * use these to modify your request before it's actually being sent
 *
 * @type {Array}
 */
...
exports.fetch = function (url, query, extendedHeaders) {
  ...
  if (pageCache.check(url) && query.cache) {
    ...
    if (exports.requestMiddleware) {
      eachOfSeries(exports.requestMiddleware, function(middleware, key, callback) {
        middleware(query, requestOptions, callback);
      }, function(err) {
        if (err) {
...
function getDocument (shouldCache, options, deferred) {
  var key = options.url||options.uri;
  if (__openRequests.hasOwnProperty(key)) {
    deferred.resolve(__openRequests[key]);
    return;
  }

  __openRequests[key] = deferred.promise;
  request(options, function (err, response, body) {
    if (err || response.statusCode !== 200) {
      delete(__openRequests[key]);
...
function map (query, extendedHeaders, callback) {
  var promises        = [],
      mappedContainer = {},
      getResultSet,
      toPush,
  ...
  getResultSet = function (mapTo, query) {
    ...
    return exports[query.type].fetch(query.url, query.map[mapTo], extendedHeaders)
            .then(function (result) {
